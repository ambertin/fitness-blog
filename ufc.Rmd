...
rousey <- fights_women %>%
  ...
  arrange(date) %>%
  mutate(won = c(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE),
         fight = 1:8) %>%
  relocate(R_fighter, B_fighter, won, fight)
...
R_data <- rousey %>%
  select(won, "fight":"B_Weight_lbs", "B_age") %>%
  slice(1:7) %>%
  rename_all(~stringr::str_replace(.,"B_",""))

B_data <- rousey %>%
  select("won":"weight_class", "R_avg_KD":"R_age", -B_age) %>%
  slice(8) %>%
